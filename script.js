function buscar() {
    ...
    //funçao q carrega as informações do JSON
    fetch('https://raw.githubusercontent.com/livianabelly/minhaAPI/refs/heads/main/dados.json').then(resposta => resposta.json()).then(banco =>{
        ...
        banco.forEach(pessoa => {
            if (identificador == pessoa.cpf) {
                //JQUERY
                var linkSite = `<a href="${pessoa.link}"target="_blank">Visite no site</a>`;
